<template>
    <div class="personal-info">
        ...
        <van-form ref="form" :model="formData" label-width="80px">
            ...
            <van-field v-model="formData.name" label="姓名" placeholder="请输入姓名" :rules="[
                    { required: true, message: '姓名不能为空' },
                    { validator: validateChineseName, message: '姓名格式不正确' }
                ]" />
            <van-field v-model="formData.idcard" label="身份证" placeholder="请输入身份证号码" :rules="[
                    { required: true, message: '身份证号码不能为空' },
                    { validator: validateIdCard, message: '身份证号码格式不正确' }
                ]" />
            <van-field v-model="formData.phone" label="手机号" placeholder="请输入手机号码" :rules="[
                    { required: true, message: '手机号码不能为空' },
                    { validator: validatePhone, message: '手机号码格式不正确' }
                ]" />
        </van-form>
        <div style="margin: 16px;">
            <van-button square block type="info" native-type="submit" @click="onSave">保存</van-button>
            <van-button square block type="default" native-type="submit" @click="onExist">退出</van-button>
        </div>
        <BottomNavigationBar />
